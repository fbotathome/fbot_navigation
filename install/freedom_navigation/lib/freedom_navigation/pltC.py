...
def pltC(C, Q, P):
    ...
    plt.plot(W[0,:], W[1,:], label='Adequação da curva de Bézier', linestyle=':', color='r')
    plt.plot(C[0,:], C[1,:], label='handle')
    plt.plot(Q[0,:], Q[1,:], label='Caminho ensinado', color='b')
    # plt.plot(P[0,:], P[1,:], 'x')
    plt.plot(C[0,:], C[1,:], 'o', label='control_points')
    return plt.gcf()
